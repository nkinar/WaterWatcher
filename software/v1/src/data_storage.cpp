...
{
    ds.start_time_str = get_time();                     // starting time
    get_turbidity(ds.ntu_voltage, ds.ntu);              // read turbidity
    get_water_temperature(ds.T_deg_C, ds.temp_good);    // read temperature
    get_tds(ds.tds_voltage, ds.ppm, ds.T_deg_C);        // obtain total dissolved solids
...
void populate_data_second()
{
  ...
  ds.end_time_str = get_time();                         // ending time
} // end
...
/*
 * Function to print the output of all science sensors (not the GPS sensor).
 * This function is for debugging only.
 */
void print_all_sensors_debug()
{
  populate_data_first();
  printSerial("---TURBIDITY SENSOR---");
  printSerial("Voltage:");
  printSerial(String(ds.ntu_voltage));
  printSerial("NTU:");
  printSerial(String(ds.ntu));
  printSerial("----------------------");

  printSerial("---Water Temperature---");
  printSerial("Temperature (C):");
  printSerial(String(ds.T_deg_C));
  printSerial("IS GOOD: ");
  printSerial(String(ds.temp_good));
  
  printSerial("---TDR---");
  printSerial("Voltage:");
  printSerial(String(ds.tds_voltage)); 
  printSerial("PPM:"); 
  printSerial(String(ds.ppm));
  printSerial("----------------------");
} // end 
