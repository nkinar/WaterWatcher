...
void modp_itoa10u(int32_t value, char* str)
{
    sprintf(str, "%" PRIu32, value);
} // end
...
void jwObj_uint( JWC_DECL char *key, uint32_t value )
{
    modp_itoa10u( value, JWC(tmpbuf) );
    jwObj_raw( JWC_PARAM key, JWC(tmpbuf) );
}
...
void jwArr_uint( JWC_DECL uint32_t value )
{
    modp_itoa10u( value, JWC(tmpbuf) );
    jwArr_raw( JWC_PARAM JWC(tmpbuf) );
}
